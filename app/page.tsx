...
export default function Home(){
  ...
  const profileSummary=`${info.name}’s responses most strongly support ${strengths.map(s=>s.toLowerCase()).join(', ')}. ${strengths.map(s=>sentence(s,scores[s])).join(' ')} ${type?`The descriptive theme “${type.name}” fits because the related answers form one of the clearest patterns in this assessment. It is a conversation starter, not an identity.`:'No archetype was assigned because the answers did not show a pattern strong and consistent enough to justify one.'}`;
  const reportContent={summary:profileSummary,learning:`${sentence('Learning Style',scores['Learning Style'])} ${scores['Learning Style']>=58?'Seeing the movement and then trying it shortly afterward may help. A coach can demonstrate one skill, give one clear instruction, and let the player repeat it immediately.':'The answers do not point clearly to only one way of learning. Combine a short explanation, a visual demonstration, and a chance to try the skill.'}`,communication:`${sentence('Communication',scores.Communication)} Give the player space to ask a question or repeat the instruction in their own words.`,pressure:voice.pressure,mistakes:voice.mistakes,focus:voice.focus,role:voice.role,leadership:voice.leadership,teamwork:`${sentence('Teamwork',scores.Teamwork)} ${sentence('Adaptability',scores.Adaptability)}`,parent:voice.parent,priorities:priorities.map(k=>({name:k,text:`Choose one small, visible ${k.toLowerCase()} behavior before each session and review one successful example afterward.`}))};
  async function submit(){ if(!info.name||!info.email||answered<20)return; setSending(true); setStep("report"); window.scrollTo({top:0,behavior:"smooth"});
    try{const res=await fetch('/api/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({player:info,answers,questions:questions.map(q=>q.text),scores,archetype:type?.name??null,archetypeMatches:matches.slice(0,4).map(m=>({name:m.name,match:m.match,description:m.description})),report:reportContent,completedAt:new Date().toISOString()})}); const data=await res.json(); setSent(data.emailSent?"Your report was emailed successfully, and Coach Al was notified.":"Your report is ready to download. Email delivery needs attention.");}catch{setSent("Your report is ready to download.");}finally{setSending(false)} }
  async function downloadPdf(){
    ...
    const margin=48,width=516; let y=0;
    const page=()=>{doc.addPage();doc.setTextColor(24);y=58};
    const heading=(title:string)=>{if(y>700)page();doc.setFont('helvetica','bold');doc.setFontSize(18);doc.setTextColor(24);doc.text(title,margin,y);y+=26};
    const paragraph=(text:string)=>{doc.setFont('helvetica','normal');doc.setFontSize(10);doc.setTextColor(72);const lines=doc.splitTextToSize(text,width);if(y+lines.length*14>744)page();doc.text(lines,margin,y,{lineHeightFactor:1.4});y+=lines.length*14+16};
    ...
    page();heading(type?.name??'Developing Profile');paragraph(profileSummary);heading('Archetype mix');closeMatches.forEach((match,index)=>paragraph(`${index===0?'Leading match':'Also present'} · ${match.name} (${match.match}% relative match)\n${match.description}`));
    heading('Learning & communication');paragraph(reportContent.learning);paragraph(reportContent.communication);
    heading('Mental game');paragraph(`Under pressure: ${voice.pressure}`);paragraph(`After a mistake: ${voice.mistakes}`);paragraph(`Staying focused: ${voice.focus}`);
    heading('Team dynamics');paragraph(`Likely role: ${voice.role}`);paragraph(`Leadership: ${voice.leadership}`);paragraph(reportContent.teamwork);
    heading('Support at home');voice.parent.forEach(item=>paragraph(`• ${item}`));paragraph('Use the archetype as a conversation starter, not a permanent description.');
    heading('Development opportunities');priorities.forEach((k,i)=>paragraph(`${i+1}. ${k}: Choose one small, visible behavior before each session and review one successful example afterward. Progress can show up through fewer reminders, quicker recovery, or more confident use in game-like moments.`));
    heading('Closing note');paragraph(`This assessment gives ${info.name}, family and coaches a shared starting point. Test the ideas in training, notice what changes, and reassess with fresh evidence over time.`);doc.save(`${info.name.replace(/[^a-z0-9]/gi,'-').toLowerCase()}-player-profile.pdf`);
  }
